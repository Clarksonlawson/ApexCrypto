...
<div class="d-flex justify-content-center align-items-center min-vh-100">
    <div class="col-lg-3">
        <div class="wg-box style-1 gap20 bg-Gainsboro shadow-none mb-32 box-about text-center">
            ...
            {{-- Section Title --}}
            <div class="head label-01 text-center mb-3">Verify Your Email Address</div>

            {{-- Status Section --}}
            <div class="flex flex-column items-center gap-3 mb-3 justify-content-center">

                {{-- SUCCESS STATUS --}}
                @if (session('status') === 'success')
                    <div class="box-status bg-YellowGreen d-inline-block px-4 py-2 rounded mb-2">
                        <i class="icon icon-check me-1"></i>
                        <span class="font-poppins fw-bold text-dark">Verification email sent successfully!</span>
                    </div>
                @endif

                {{-- ERROR STATUS --}}
                @if (session('status') === 'error')
                    <div class="box-status bg-Red d-inline-block px-4 py-2 rounded mb-2">
                        <i class="icon icon-close me-1"></i>
                        <span class="font-poppins fw-bold text-white">{{ session('error_detail') ?? 'Something went wrong.' }}</span>
                    </div>
                @endif

                {{-- DEFAULT STATUS --}}
                @if (!session('status'))
                    <div class="box-status bg-LightGray d-inline-block px-4 py-2 rounded mb-2">
                        <i class="icon icon-info me-1"></i>
                        <span class="font-poppins">Verification pending.</span>
                    </div>
                @endif
            </div>
            ...
            {{-- Email Info --}}
            @if (session('email'))
                <div class="number-exchange f12-medium text-center text-GrayDark mt-2">
            ...
            {{-- Description --}}
            <div class="desc f12-medium text-GrayDark text-center mt-3">
                {{ session('message') ?? 'If you registered, please check your email and click the link to verify your account. If you didn’t receive the email, click below to request another.' }}
            </div>

            {{-- Resend Link --}}
            <a href="" class="btn-read-more f12-bold mt-3 d-inline-block">
                Resend Verification Link <i class="icon icon-send"></i>
            </a>
        </div>
    </div>
</div>
